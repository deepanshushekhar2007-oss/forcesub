...
async def smart_group_otp_detect(message: Message):
    ...
    # 📱 Extract all digits from full message
    all_digits = re.findall(r"\d", text)

    if len(all_digits) < 3:
        return

    # last 3 digits of masked number
    last3 = "".join(all_digits[-9:-6]) if len(all_digits) > 6 else "".join(all_digits[-3:])

    # Better way: find number line specifically
    lines = text.split("\n")
    number_line = None
    ...
    for user_id, data in list(db["active"].items()):
        ...
        if real_number.endswith(last3):
            ...
            try:
                ...
                # Remove permanently
                if real_number in db["countries"].get(country, []):
                    db["countries"][country].remove(real_number)

                db["locked"].pop(real_number, None)
                db["active"].pop(user_id, None)
