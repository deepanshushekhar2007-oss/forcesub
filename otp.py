...
@dp.message()
async def stylish_multi_user_otp_detect(message: Message):
    """
    Detect OTPs from stylish single-line group messages and forward them to correct users.
    Works for multiple active users and both masked/full numbers.
    Example message:
    ...
    if not db.get("active"):
        print("No active users, skipping message.")
        return

    print("GROUP MESSAGE:", text)
    print("ACTIVE USERS:", db.get("active"))

    # 1️⃣ Extract all OTPs in the message (after 🔑 or fallback)
    ...
    # 2️⃣ Extract all digit sequences (numbers) from message
    all_numbers_digits = re.findall(r"\d", text)
    if len(all_numbers_digits) < 3:
        print("No valid number detected")
        return

    # 3️⃣ Match each active user
    delivered = 0
    for user_id, data in list(db["active"].items()):
        real_number_digits = "".join(re.findall(r"\d", data["number"]))
        last3 = real_number_digits[-3:]

        # Check if last3 is anywhere in the message digits
        if "".join(all_numbers_digits).find(last3) != -1:
            try:
                # Assign OTP to user (first match for simplicity)
                otp = otp_matches[0]
    ...
    if delivered == 0:
        print("No OTP delivered. Check active numbers and message format.")
