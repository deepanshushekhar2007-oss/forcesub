...
async def admin_panel(message: Message):
    ...
    settings = await mongo_db["settings"].find_one({"_id": "bot"}) or {}
...
async def upload_numbers(message: Message):
    ...
    if not data:
        return await message.answer("❌ Please select country first")
...
async def live_stock(message: Message):
    ...
    for country in countries:
        ...
        total = numbers_db.count_documents({"country": country})

        locked = numbers_db.count_documents({
            "country": country,
            "locked": True
        })
...
async def broadcast(message: Message):
    ...
    users = list(users_db.find({}, {"user_id": 1}))
