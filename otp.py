...
async def start(message: Message):
    ...
    try:
        ...
        if user_data and not user_data.get("notified") and not is_admin(user.id):
...
async def upload_numbers(message: Message):
    ...
    try:
        ...
        await message.answer(
            ...
            f"📥 Numbers Found: <b>{len(raw_numbers)}</b>\n"
            f"✅ Numbers Added: <b>{added}</b>"
        )
        ...
        await message.answer("❌ Error processing TXT file")
...
async def cancel_remove_country(call: CallbackQuery):
    ...
    await call.message.edit_text("❌ Country removal cancelled")

# required imports if not already
import time
import random
...
async def select_country(call: CallbackQuery):
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=c,
                callback_data=f"country_{c.replace(' ', '_')}"
            )
        ]
        for c in countries
    ]
...
async def give_number(call: CallbackQuery):
    ...
    country = call.data.replace("country_", "").replace("_", " ")
    ...
    # ---------- Fetch numbers async ----------
    numbers_cursor = numbers_db.find({"country": country})

    numbers = []
    async for doc in numbers_cursor:
        num = doc.get("number")
        if num:
            numbers.append(num)
    ...
    if time.time() - last < 5:
        ...
        return await call.answer(
            f"⏳ Wait {wait}s before changing number",
            show_alert=True
        )

    db["cooldown"][user_id] = time.time()

    # ---------- Remove old active numbers ----------
    old = db["active"].get(user_id)

    if old:

        old_numbers = old.get("numbers", [])
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="🔄 Change Numbers",
                callback_data=f"country_{country.replace(' ', '_')}"
            )
        ],
...
async def change_number(call: CallbackQuery):
    ...
    for number in numbers:
        ...
        # remove from country stock
        if country and number in db.get("countries", {}).get(country, []):
    ...
    await call.answer("♻ Number Changed Successfully")
...
async def admin_reply(message: Message):
    ...
    await message.answer("✅ OTP Sent & Number Released")
...
async def stylish_otp_forward(event):
    ...
    # 🔥 SPLIT INTO BLOCKS (multi OTP support)
    blocks = text.split("\n\n")
    ...
    for block in blocks:
        ...
        for user_id, data in list(db["active"].items()):
            ...
            for number in numbers:
                ...
                user_num = "".join(re.findall(r"\d", number))
                ...
                for candidate in number_candidates:
                    ...
                    cand = "".join(re.findall(r"\d", candidate))

                    if len(cand) < 6:
                        continue

                    # 🔥 UNIVERSAL MATCH (NO FAIL)
                    if (
                        cand.startswith(start)
                        and (
                            cand.endswith(end3)
                            or cand.endswith(end2)
                        )
                    ):
