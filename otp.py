...
async def upload_numbers(message: Message):
    ...
    if message.from_user.id != data["admin"] and message.from_user.id != OWNER_ID:
        return
    ...
    numbers = re.findall(r"\+?\d{6,15}", content)
    ...
    await message.answer(
        f"📄 TXT Processed\n\n✅ {added} Numbers Added in <b>{country}</b>"
    )
...
async def remove_country(message: Message):
    ...
    if country in db["countries"]:
        ...
        await message.answer(f"🗑 <b>{country}</b> Removed")
...
async def stylish_otp_forward(event):
    ...
    otp_match = re.search(r"🔑\s*(\d{4,6})", text)
