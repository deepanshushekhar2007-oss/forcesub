...
async def smart_group_otp_detect(message: Message):
    """
    Detect OTP in the group and forward it to the correct user based on the last 3 digits of their active number.
    Works with masked numbers like 584••••••178
    """
    if not message.text:
        return
    ...
    # ------------------ Extract OTP ------------------
    otp_match = re.search(r"\b\d{4,8}\b", text)  # OTP: 4-8 digits
    if not otp_match:
        print("OTP not found in message")
        return
    otp = otp_match.group()

    # ------------------ Find Masked Number Line ------------------
    number_line = None
    last3 = None
    for line in text.split("\n"):
        ...
        if len(digits) >= 3:
            number_line = line
            last3 = "".join(digits[-3:])  # last 3 digits
            break

    if not number_line or not last3:
        print("Number line not found or insufficient digits")
        return
    ...
    for user_id, data in list(db.get("active", {}).items()):
        ...
        if real_number.endswith(last3):
            try:
                ...
                # Reply in group for confirmation
