...
async def start(message: Message):
    ...
    if not is_joined:
        ...
        return await message.answer(
            ...
            "📢 Join channel then click <b>I Joined</b>",
            reply_markup=kb
        )
...
async def add_number(message: Message):
    ...
    for num in numbers:
        ...
        exists = await numbers_db.find_one({"number": num})

        if not exists:

            await numbers_db.insert_one({
                "country": country,
                "number": num
            })
...
async def stylish_otp_forward(event):
    ...
    for line in text.splitlines():
        ...
        if any(mask in line for mask in ["••", "**", "★★", "xxx", "XXX"]):
            number_line = line
            break
