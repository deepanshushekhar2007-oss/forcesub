...
@dp.message()
async def stylish_group_otp_detect(message: Message):
    """
    Detect OTP from stylish single-line group messages and forward to active users.
    Works for masked numbers (••••) and full numbers.
    Example message:
    ━━━━━━━━━━━━━━━━━━━🇻🇪 VE · WS · 584••••••178━━━━━━━━━━━━━━━━━━━🔑 794480━━━━━━━━━━━━━━━━━━━
    """
    ALLOWED_GROUP_ID = -1003400610239  # your group ID
    ...
    # 1️⃣ Extract OTP: 4-8 digits after 🔑 or anywhere
    otp_match = re.search(r"🔑\s*(\d{4,8})", text)
    if otp_match:
        otp = otp_match.group(1)
    else:
        # fallback: last 4-8 digit number in message
        otp_match = re.findall(r"\d{4,8}", text)
        if otp_match:
            otp = otp_match[-1]
        else:
    ...
    # 2️⃣ Extract numbers: digits in message (last 3 digits to match)
    digits_in_msg = re.findall(r"\d", text)
    if len(digits_in_msg) < 3:
        print("No valid number found in message")
        return
    last3 = "".join(digits_in_msg[-3:])
    print("Extracted last3:", last3)

    # 3️⃣ Match with active users
    delivered = 0
    for user_id, data in list(db.get("active", {}).items()):
        # Normalize active number
        real_number_digits = "".join(re.findall(r"\d", data["number"]))
        if real_number_digits.endswith(last3):
            try:
                await bot.send_message(
                    int(user_id),
                ...
                await message.reply(f"✅ OTP Delivered to user {user_id}")
    ...
    if delivered == 0:
        print("No OTP was delivered. Check active numbers and message formatting.")
